...
import (
	...
	"github.com/Safrochain_Org/safrochain/testutil/setup"
)
...
func (suite *KeeperTestHelper) Setup() {
	...
	dir, err := os.MkdirTemp("", "safrochaind-test-home")
	if err != nil {
	...
	if common.IsDebugLogEnabled() {
		suite.App = setup.Setup(false, dir, "safrochain-1", suite.T())
	} else {
		suite.App = setup.Setup(false, dir, "safrochain-1")
	}

	suite.Ctx = suite.App.NewContextLegacy(false, tmtypes.Header{Height: 1, ChainID: "safrochain-1", Time: defaultTestStartTime})
...
func (suite *KeeperTestHelper) SetupTestForInitGenesis() {
	dir, _ := os.MkdirTemp("", "safrochaind-test-home")
	// Setting to True, leads to init genesis not running
	suite.App = setup.Setup(true, dir, "safrochain-1")
