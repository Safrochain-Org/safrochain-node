...
function query_contract {
    ARGS=${3:-$safrochaind_COMMAND_ARGS}
    safrochaind query wasm contract-state smart $1 $2 --output json $ARGS
}
...
function wasm_cmd {
    ...
    ARGS=${5:-$safrochaind_COMMAND_ARGS}
    ...
    tx_hash=$(safrochaind tx wasm execute $CONTRACT $MESSAGE $FUNDS $ARGS | jq -r '.txhash')
...
function mint_cw721 {
    ...
    TXMINT=$($BINARY tx wasm execute "$CONTRACT_ADDR" "$EXECUTED_MINT_JSON" $safrochaind_COMMAND_ARGS | jq -r '.txhash') && echo $TXMINT
}