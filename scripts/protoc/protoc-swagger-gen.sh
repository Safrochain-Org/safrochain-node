...
# Delete cosmos/mint path since safrochain uses its own module
...
for f in $files; do
  ...
  case "$f" in
    ...
    *safrochain*) cp "$f" ./tmp-swagger-gen/_all/safrochain-$counter.json ;;
...
base_json=$(jq -n --arg version "$version" '{
  swagger: "2.0",
  info: { title: "safrochain network", version: $version },
